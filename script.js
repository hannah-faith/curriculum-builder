...
function initCourseDetails() {
  ...
  // Initial import
  const vocabRaw =
    course.vocabulary && typeof course.vocabulary === "string" ? course.vocabulary : "";
  ...
  const languageField = Array.from(cardBody.querySelectorAll(".field-group")).find(
    (fg) => fg.querySelector("label") && fg.querySelector("label").textContent === "Language"
  );
...
function renderRubric() {
  ...
  course.rubric.forEach((item, idx) => {
    ...
    textareaSubtitle.addEventListener("input", () => {
      item.subtitle = textareaSubtitle.value;
    });
    ...
    ["Move Up", "Move Down", "Delete Item"].forEach((text) => {
      ...
      btn.addEventListener("click", () => {
        ...
        else if (text === "Move Up" && idx > 0)
          [course.rubric[idx - 1], course.rubric[idx]] = [
            course.rubric[idx],
            course.rubric[idx - 1]
          ];
        else if (text === "Move Down" && idx < course.rubric.length - 1)
          [course.rubric[idx + 1], course.rubric[idx]] = [
            course.rubric[idx],
            course.rubric[idx + 1]
          ];
  ...
  rubricDrake = window
    .dragula(reqContainers, {
      moves: (el, source, handle) => handle.classList.contains("drag-handle")
    })
    .on("drop", (el, target) => {
      const rubricIdx = Array.from(document.querySelectorAll(".rubric-item")).indexOf(
        target.closest(".rubric-item")
      );
...
function renderSectionGroups() {
  ...
  course.section_groups.forEach((grp, gidx) => {
    ...
    const { card, header, body } = createCard(
      ...
      sec.steps.forEach((step, stidx) => {
        ...
        ["↑", "↓", "×"].forEach((sym) => {
          ...
          btn.addEventListener("click", () => {
            ...
            if (sym === "↑" && stidx > 0)
              [arr[stidx - 1], arr[stidx]] = [arr[stidx], arr[stidx - 1]];
            if (sym === "↓" && stidx < arr.length - 1)
              [arr[stidx + 1], arr[stidx]] = [arr[stidx], arr[stidx + 1]];
            renderSectionGroups();
          });
          stepCtrl.append(btn);
    ...
    ["↑", "↓", "×"].forEach((txt) => {
      ...
      btn.addEventListener("click", () => {
        if (txt === "×") course.section_groups.splice(gidx, 1);
          ...
          [course.section_groups[gidx - 1], course.section_groups[gidx]] = [
            course.section_groups[gidx],
            course.section_groups[gidx - 1]
          ];
        else if (txt === "↓" && gidx < course.section_groups.length - 1)
          [course.section_groups[gidx + 1], course.section_groups[gidx]] = [
            course.section_groups[gidx],
            course.section_groups[gidx + 1]
          ];
...
document.getElementById("add-rubric-item").addEventListener("click", () => {
  course.rubric.push({ id: genUUID(), type: "", title: "", requirements: [] });
  renderRubric();
});
document.getElementById("add-section-group").addEventListener("click", () => {
  course.section_groups.push({ id: genUUID(), title: "", sections: [] });
  renderSectionGroups();
});
...
document.getElementById("import-json-header").addEventListener("change", (event) => {
  ...
  reader.onload = (e) => {
    try {
      ...
      if (
        data.activities &&
        data.activities["checkpoint-set"] &&
        Array.isArray(data.activities["checkpoint-set"].checkpoints)
      ) {
      ...
      Object.assign(course, {
        ...
        vocabulary:
          typeof data.vocabulary === "string" || Array.isArray(data.vocabulary)
            ? data.vocabulary
            : [],
        rubric: Array.isArray(data.rubric) ? data.rubric : [],
        section_groups: Array.isArray(data.sectionGroups)
          ? data.sectionGroups
          : Array.isArray(data.section_groups)
          ? data.section_groups
          : [],
...
const jsonPanel =
  document.getElementById("json-panel") || document.getElementById("json-preview-panel");
const toggleJsonBtn =
  document.getElementById("json-toggle") || document.getElementById("toggle-json-preview");
...
function syncActivities() {
  const ids = new Set();
  course.section_groups.forEach((g) =>
    g.sections.forEach((sec) =>
      sec.steps.forEach((st) => {
  ...
  ids.forEach((id) => {
    if (!course.activities[id]) course.activities[id] = {};
  });
...
function renderScoring() {
  ...
  course.scoring.criteria.forEach((crit, idx) => {
    ...
    lblT.textContent = "Title";
    const inpT = document.createElement("input");
    inpT.type = "text";
    inpT.value = crit.title || "";
    inpT.addEventListener("input", () => (crit.title = inpT.value));
    fgT.append(lblT, inpT);
    body.append(fgT);
        ...
        if (comp.variant === "rubric") {
          ...
          comp.categories.forEach((cat, catIdx) => {
            ...
            inputCat.addEventListener("input", () => {
              comp.categories[catIdx] = inputCat.value;
            });
    ...
    crit.contexts.forEach((ctx, kidx) => {
      ...
      addMemBtn.textContent =
        ctx.memberKey === "activities" ? "+ Add Activity" : "+ Add Checkpoint";
...
function renderActivities() {
  ...
  Object.entries(course.activities).forEach(([id, obj]) => {
    ...
    // Tool selector
    const fgTool = document.createElement("div");
    fgTool.className = "field-group";
    ...
    fgMedia.className = "field-group";
    const lblMedia = document.createElement("label");
    lblMedia.textContent = "Media Type";
    ...
    // Action field
    const fgAction = document.createElement("div");
    fgAction.className = "field-group";
    const lblAction = document.createElement("label");
    lblAction.textContent = "Action";
    ...
    if (obj.tool === "checkpoint-set") {
      ...
      obj.checkpoints.forEach((cp, cpidx) => {
        ...
        // Variant
        addSelect(cpBody, "Variant", cp, "variant", [
          "none",
          "multiple_choice",
          "short_answer",
          "rubric",
          ...
          function renderQFields() {
            ...
            switch (qb.type) {
              ...
              case "code":
                ...
                addSelect(qbBody, "Language", qb, "language", [
                  "python",
                  "java",
                  "html",
                  "css",
              ...
              case "media":
                ...
                const inpQH = document.createElement("input");
                inpQH.type = "number";
                inpQH.value = qb.height ? parseInt(qb.height) : "";
                inpQH.addEventListener(
                  "input",
                  () => (qb.height = inpQH.value ? inpQH.value + "px" : "")
                );
                fgQH.append(lblQH, inpQH);
                qbBody.append(fgQH);
                break;
            }
          }
        ...
        addQB.type = "button";
        addQB.textContent = "+ Add Question Part";
        addQB.addEventListener("click", () => {
          cp.question.push({ id: genUUID(), type: "text", text: "" });
          renderActivities();
        });
        qCont.append(addQB);
        cpBody.append(qCont);
        ...
        cp.choices.forEach((ch, cidx) => {
          ...
          function renderChFields() {
            ...
            switch (ch.type) {
              ...
              case "code":
                ...
                addSelect(chBody, "Language", ch, "language", [
                  "python",
                  "java",
                  "html",
                  "css",
              ...
              case "media":
                ...
                inpCH.addEventListener(
                  "input",
                  () => (ch.height = inpCH.value ? inpCH.value + "px" : "")
                );
                fgCH.append(lblCH, inpCH);
                chBody.append(fgCH);
                break;
            }
          ...
          remCh.type = "button";
          remCh.textContent = "Remove Choice";
          remCh.addEventListener("click", () => {
            cp.choices.splice(cidx, 1);
            renderActivities();
          });
          chBody.append(remCh);
        ...
        addCh.addEventListener("click", () => {
          cp.choices.push({ id: genUUID(), type: "text", text: "", correct: false });
          renderActivities();
        });
        chCont.append(addCh);
        ...
        cp.explanation.forEach((ex, eidx) => {
          ...
          remEx.addEventListener("click", () => {
            cp.explanation.splice(eidx, 1);
            renderActivities();
          });
          exBody.append(remEx);
        ...
        addEx.type = "button";
        addEx.textContent = "+ Add Explanation";
        addEx.addEventListener("click", () => {
          cp.explanation.push({ id: genUUID(), type: "text", text: "" });
          renderActivities();
        });
        exCont.append(addEx);
        ...
        remCp.type = "button";
        remCp.textContent = "Remove Checkpoint";
        remCp.addEventListener("click", () => {
          obj.checkpoints.splice(cpidx, 1);
          renderActivities();
        });
        cpBody.append(remCp);
      ...
      addCp.type = "button";
      addCp.textContent = "+ Add Checkpoint";
    ...
    // Raw JSON editor
    const fgRaw = document.createElement("div");
  ...
  bg.blocks.forEach((blk, bix) => {
    ...
    ["↑", "↓", "×"].forEach((sym) => {
      ...
      btn.addEventListener("click", () => {
        ...
        if (sym === "↑" && bix > 0)
          [bg.blocks[bix - 1], bg.blocks[bix]] = [bg.blocks[bix], bg.blocks[bix - 1]];
        if (sym === "↓" && bix < bg.blocks.length - 1)
          [bg.blocks[bix + 1], bg.blocks[bix]] = [bg.blocks[bix], bg.blocks[bix + 1]];
        renderSectionGroups();
      });
      ...
      switch (blk.type) {
        ...
        case "checkpoint":
            ...
            function renderQBFields() {
              ...
              switch (qb.type) {
                ...
                case "code":
                  ...
                  addSelect(qbFields, "Language", qb, "language", [
                    "python",
                    "java",
                    "html",
                    "css",
            ...
            function renderChFields() {
              ...
              switch (choice.type) {
                ...
                case "code":
                  ...
                  addSelect(chFields, "Language", choice, "language", [
                    "python",
                    "java",
                    "html",
                    "css",
          ...
          chkSub.addEventListener("change", () => {
            blk.is_submission = chkSub.checked;
          });
          ...
          chkVal.addEventListener("change", () => {
            blk.requireValidation = chkVal.checked;
          });
          ...
          blk.table.forEach((row, ridx) => {
            ...
            ["↑", "↓", "×"].forEach((sym) => {
              ...
              btn.addEventListener("click", () => {
                ...
                if (sym === "↑" && ridx > 0)
                  [blk.table[ridx - 1], blk.table[ridx]] = [blk.table[ridx], blk.table[ridx - 1]];
                if (sym === "↓" && ridx < blk.table.length - 1)
                  [blk.table[ridx + 1], blk.table[ridx]] = [blk.table[ridx], blk.table[ridx + 1]];
                renderSectionGroups();
              });
              rowCtrl.append(btn);
